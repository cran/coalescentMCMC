...
\title{Likelihood Function of Some Time-Dependent Coalescent Models}
...
$\Theta = 4N_e\nu$, with $N_e$ the effective size of the population
and $\nu$ the mutation rate. The present document gives the likelihood
...
\exp\left[-{i \choose 2}\frac{t_i - t_{i-1}}{\Theta}\right]
\end{displaymath}
where $t_1=0$ is the present time and $t_n = \tmrca$. The general
formula for $\Theta(t)$ varying through time is:

\begin{equation}\label{eq:timecoal}
\prod_{i=2}^n {i \choose 2}\frac{1}{\Theta(t_i)}
\exp\left[-{i \choose 2}\int_{t_{i-1}}^{t_i}\frac{1}{\Theta(u)}\mathrm{d}u\right]
\end{equation}
...
- 2\times{i \choose 2}\frac{t_i - t_{i-1}}{\Theta^3},
\end{displaymath}
and:
...
\begin{displaymath}
\ln L = \sum_{i=2}^n \ln{i \choose 2}- \ln\Theta_0 - \rho t_i+{i \choose 2}\frac{1}{\rho\Theta_0}(e^{-\rho t_i} - e^{-\rho t_{i-1}}),
\end{displaymath}
with its first partial derivatives being:

\begin{eqnarray*}
\frac{\partial\ln L}{\partial\Theta_0} &=& \sum_{i=2}^n -\frac{1}{\Theta_0}-{i \choose 2}\frac{1}{\rho\Theta_0^2}(e^{-\rho t_i} - e^{-\rho t_{i-1}}),\\
\frac{\partial\ln L}{\partial\rho} &=& \sum_{i=2}^n -t_i + {i \choose
...
\ln(\Theta_0+\kappa t_i) - {i \choose 2}\frac{1}{\kappa}
